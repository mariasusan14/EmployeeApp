...
export const List = ({ employees }: { employees: Employee[] }) => {
  ...
  const [employeeIdToBeDeleted, setEmployeeIdToBeDeleted] = useState<
    number | undefined
  >(undefined);
  ...
  function handleDeleteIconClick(e: React.MouseEvent, empId: number) {
    ...
    setEmployeeIdToBeDeleted(empId);
  }
  ...
  return (
    <>
      {showDeletePopup && (
        <DeletePopup
          id={employeeIdToBeDeleted}
          setPopupState={setShowDeletePopup}
        />
      )}
      <table className="list">
        ...
        <tbody>
          {employees.map((employee) => {
            return (
              <tr
                onClick={() => handleNavigationToEmployeeDetails(employee.id)}
              >
